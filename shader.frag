...
void main(){
	...
	// --- DYNAMIC GEOMETRIC MOSH PATTERN (8-Style Mega Evolution) ---
	float shapeMask = 0.0;
	vec2 moshUv = uv;

	// GATING: Calculate a "Burst" factor based on sudden volume jumps
	...
	// Initial Global Inversion
	if(invertToggle) color.rgb = 1.0 - color.rgb;
	
	// --- CONDITIONAL PATTERN INVERSION ---
	if(invertToggle) {
		// Global Invert is ON: Background is negative.
		// We create a "Ghost Highlight" to make the geometric pattern pop
		// without turning into a gray mush.
		vec3 patternColor = abs(0.2 - color.rgb);
		
		// Blend factor at 0.4 for a subtle, high-end glass feel
		color.rgb = mix(color.rgb, patternColor, shapeMask * 0.4);
	} else {
		// Global Invert is OFF: Background is normal.
		// No color inversion applied here—just keep the original video colors.
		// The pattern is still visible through the Mosh distortion/pixelation.
		color.rgb = color.rgb;
	}
